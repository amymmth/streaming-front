      ...
      CallExpression(path) {
        if (path.get('callee').isImport()) {
          ...
          if (moduleName.isStringLiteral()) {
            path.replaceWith(t.callExpression(t.identifier('__linaria_dynamic_import'), [t.stringLiteral(moduleName.node.value)]));
            return;
          }
          if (moduleName.isTemplateLiteral()) {
            path.replaceWith(t.callExpression(t.identifier('__linaria_dynamic_import'), [t.cloneNode(moduleName.node, true, true)]));
            return;
          }

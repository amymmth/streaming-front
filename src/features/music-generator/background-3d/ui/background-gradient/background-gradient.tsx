...
export const BackgroundGradient: FC<Props> = ({ colors, intensity,isPlaying }) => {
    ...
    const uniforms = useMemo(() => ({
        ...
        u_color7: { value: colorsRef.current.color7 },
        u_color8: { value: colorsRef.current.color8 },
        u_color9: { value: colorsRef.current.color9 },
    ...
    useFrame(({ clock }) => {
        if (meshRef.current) {
            ...
            material.uniforms.u_color7.value.lerp(colorsRef.current.color7, 0.1);
            material.uniforms.u_color8.value.lerp(colorsRef.current.color8, 0.1);
            material.uniforms.u_color9.value.lerp(colorsRef.current.color9, 0.1);
        }
    });
